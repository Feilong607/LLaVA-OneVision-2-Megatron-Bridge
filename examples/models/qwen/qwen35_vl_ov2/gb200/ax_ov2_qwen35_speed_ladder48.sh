#!/usr/bin/env bash
# =============================================================================
# Qwen3.5-35B-A3B merged video stage — 48-GPU SPEED LADDER (A -> T in one workload, no extra variables).
#
# Runs ax_ov2_qwen35_merged64k_smoke.sh twice on the same 12 pods, 80 iterations per
...
#   TP2 full, alltoall           ~173  OOM by ~11 IF the model's TP2 logits term (90 GiB, a fit) is right;
#                                      HybridEP cap 36864 > 21824 so TP2 can only run ACCEL=0
...
#
# Round 1 (this file): two legs, CE fusion off, same 80 iters, same per-rank work (GBS = 4 x DP):
#   A  TP4/ETP2 GBS48 full recompute, ACCEL=2 HybridEP   = the 09-13 memory-smoke config, now 80 iters
#      -> late-window throughput for this blend; the 20-step reference has a different LR schedule
#   T  TP2/ETP2 GBS96 full recompute, ACCEL=0 alltoall   -> does TP2 fit at all (model says no by ~11 GiB,
#      a clean torch OOM at step 1 costs ~15 min), and if it fits, is halving TP comm worth losing HybridEP
#   Throughput is compared as samples/s (GBS / mean Step Time), never as s/iter: T's GBS is 2x A's.
#   speedup(T vs A) = (96 / T_s) / (48 / A_s) = 2 * A_s / T_s.
# Round 2 (later, recipe decisions): selective recompute needs a real memory lever -- a chunked CE that never
#   materialises fp32 logits, or dropping the MTP head (halves the logits term: ~22 GiB at TP4, ~45 at TP2).
#
...
# Exit: 0 = both legs passed, 1 = a candidate failed, 2 = summary failure, 3 = coordination failure.
#
...
export OV2_CE_FUSION="${OV2_CE_FUSION:-false}"   # see header: not a memory lever, unvalidated through the MTP head
...
# Reference logs for the reproducibility table: the 09-13 48-GPU memory smoke (20-iteration run; its LR decay schedule differs from this 80-iteration run).
...
if [[ -e "$_L_OUT" || -e "$(_result_of A)" || -e "$(_result_of T)" || -d "$HOME/ckpts_video_sft/_smoke_qwen35_merged64k/${_L_TAG}-A" || -d "$HOME/ckpts_video_sft/_smoke_qwen35_merged64k/${_L_TAG}-T" ]]; then
...
# ---------------- T: TP2 full recompute, alltoall (fits? faster?) ----------------
export OV2_RECOMPUTE_FULL=1 OV2_RECOMPUTE_MOE=0 OV2_VISION_RECOMPUTE=1
_leg T 2 96 0
...
if (( _L_IS_MASTER )); then
  _tmp="$_L_OUT.$$"
  {
    ...
    echo "reference (09-13 memory smoke, 20 iters, different LR-decay length): max_allocated 115.2 GiB, ~100 s/iter during warm-up"
    echo
    echo "---- throughput (samples/s = GBS / mean of last 20 Step Time; s/iter is NOT comparable across legs) ----"
    echo "A: $(_tput A 48)"
    echo "T: $(_tput T 96)"
    _ma="$(_mean_step A)"; _mt="$(_mean_step T)"
    ...
    echo
    echo "---- descriptive loss comparison (20-step and 80-step LR schedules differ): first 5 lm loss, 09-13 reference vs leg A (loose check only; not a numerics acceptance) ----"
...
n = min(len(r), len(a))
print(f"max |diff| over first {n}: {max(abs(x-y) for x,y in zip(r[:n],a[:n])):.4f}  (descriptive only: input identity and LR schedule are not matched; NOT numerical acceptance)" if n else "max |diff|: n/a (missing series)")
PY
    for leg in A T; do
      echo; echo "---- leg $leg ----"
      case "$leg" in
        A) echo "TP4/ETP2 GBS48 (DP12), full recompute, ACCEL=2 HybridEP, ce_fusion=$OV2_CE_FUSION";;
        T) echo "TP2/ETP2 GBS96 (DP24), full recompute, ACCEL=0 alltoall, ce_fusion=$OV2_CE_FUSION  [per-rank work = 4 microbatches, same as A]";;
      esac
      r="$(_result_of "$leg")"
