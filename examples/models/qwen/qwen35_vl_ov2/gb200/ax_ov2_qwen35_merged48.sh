...
#   budget      1 epoch of the blend = 966,252 bins = 20,130 iters @ GBS48 (OV2_MIDTRAIN_N_SAMPLES overrides)
#
...
#
# First launch: fresh SAVE, weights from INIT_CKPT, full budget. Restart: same Args + same SAVE (+ Workers=11);
# Bridge restores model, Muon, scheduler, iteration, RNG and Energon state. Never shrink the budget on restart.
# Changing TP/ETP/DP or the recompute lane mid-run = new SAVE + INIT_CKPT=<last complete iter> (weights only).
#
...
try:
    ...
    import torch
    import yaml
    step = int(torch.load(tracker, map_location="cpu", weights_only=True)["step"]) if tracker.exists() else int(legacy.read_text().strip())
    if step <= 0:
...
_say "blend=$DATA_PATH ($_n_ds dirs, total weight $_TOTAL_W) n_samples=$OV2_MIDTRAIN_N_SAMPLES iters=$_ITERS warmup=$_WARMUP save_every=$SAVE_EVERY keep=${OV2_KEEP_CKPTS:-4}"
