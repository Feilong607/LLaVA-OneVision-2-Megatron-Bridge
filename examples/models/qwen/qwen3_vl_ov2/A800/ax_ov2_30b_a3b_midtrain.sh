...
# A800-80G TP=1 fit (DEFAULT): TP=1 structurally avoids the MoE-combine SP reduce-scatter wedge
# (token_dispatcher.py:803, gated on tp_size>1). To fit the unsharded 30B + ~86-frame video vision
# activation, force FULL recompute on BOTH the LLM and the vision tower (OV2_VISION_RECOMPUTE=1 ->
# llava_ov2 keeps vis_cfg.recompute_granularity=full) + optimizer CPU offload. AdamW (not Muon) for
# the full MoE (Muon EP-deadlocks). Override OV2_OFFLOAD_FRACTION= / OV2_VISION_RECOMPUTE= to retune.
if [[ "$TP" == "1" ]]; then
  OV2_VISION_RECOMPUTE="${OV2_VISION_RECOMPUTE:-1}"
  RECOMPUTE_ARGS="model.recompute_granularity=full model.recompute_method=uniform model.recompute_num_layers=1"
  OFFLOAD_ARGS="optimizer.optimizer_cpu_offload=true optimizer.optimizer_offload_fraction=${OV2_OFFLOAD_FRACTION:-1.0}"
  # offload REQUIRES use_precision_aware_optimizer=True: mcore HybridDeviceOptimizer seeds the CPU fp32
  # master from param_to_fp32_param, populated ONLY on the precision-aware path. Without it the CPU
  # master is never seeded from the ckpt -> first step zeros the weights -> iter~3 forward NaN
  # (NVIDIA/Megatron-LM #1842/#1872/#1986). Pair them automatically here (dtypes stay fp32 -> no precision change).
  OV2_PRECISION_AWARE="${OV2_PRECISION_AWARE:-1}"
else
  OV2_VISION_RECOMPUTE="${OV2_VISION_RECOMPUTE:-0}"; RECOMPUTE_ARGS=""; OFFLOAD_ARGS=""
  OV2_PRECISION_AWARE="${OV2_PRECISION_AWARE:-0}"
fi
# OV2_RECOMPUTE_FULL=1 -> full LLM activation recompute (granularity=full/uniform/1, ~13GB saved)
# instead of the default selective core_attn. Needed for the TP=1 dropless path: TP=1 AVOIDS the
# MoE-combine SP reduce-scatter wedge (token_dispatcher.py combine_preprocess is gated on tp_size>1)
# but keeps vision/attn activations unsharded -> needs full recompute to fit 80GB. Auto-on at TP=1,
# off at TP>1 (TP>1 fits via capacity-drop instead); override explicitly if desired.
if [[ "$TP" == "1" ]]; then OV2_RECOMPUTE_FULL="${OV2_RECOMPUTE_FULL:-1}"; else OV2_RECOMPUTE_FULL="${OV2_RECOMPUTE_FULL:-0}"; fi
...
MOE_CAPACITY_FACTOR="${MOE_CAPACITY_FACTOR:-none}"   # TP=1 path is dropless (no SP reduce-scatter); AIAK-faithful
...
echo "[ov2-30b-midtrain] nnodes=${NNODES:-1} TP=$TP SP=$SP seq=$SEQ_LEN gbs=$MIDTRAIN_GBS iters=$ITERS moe_capacity=$MOE_CAPACITY_FACTOR pad_to_capacity=$MOE_PAD_TO_CAPACITY recompute_full=$OV2_RECOMPUTE_FULL init=$INIT_CKPT save=$SAVE (FULL model, AdamW for MoE)"
docker run -d --name ov2_30b_p16m33_mid --network=host --privileged --gpus all -e CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 \
  ...
  -e OV2_MIDTRAIN_ADAMW=${OV2_MIDTRAIN_ADAMW:-1} -e OV2_MIDTRAIN_MUON=${OV2_MIDTRAIN_MUON:-0} \
  ...
  -e OV2_PRECISION_AWARE=$OV2_PRECISION_AWARE \
  -e OV2_FREEZE_LLM=${OV2_FREEZE_LLM:-0} -e OV2_FREEZE_VISION=${OV2_FREEZE_VISION:-0} -e OV2_FREEZE_ADAPTER=${OV2_FREEZE_ADAPTER:-0} \
  -e OV2_STAGE2_ADAMW=${OV2_STAGE2_ADAMW:-0} -e OV2_PACK_FULL_CAUSAL=${OV2_PACK_FULL_CAUSAL:-0} -e OV2_MOE_PERMUTE_FUSION=${OV2_MOE_PERMUTE_FUSION:-0} -e OV2_SPATIAL_MERGE=${OV2_SPATIAL_MERGE:-} \
  ...
  -v /ov2:/ov2 -v /vlm:/vlm -w "$REPO" "$IMAGE" bash -lc "
    python -m torch.distributed.run $RDZV --nproc_per_node=$NPROC scripts/training/run_recipe.py \
      ...
      model.tensor_model_parallel_size=$TP model.sequence_parallel=$SP $MOE_CAPACITY_ARGS $RECOMPUTE_ARGS $OFFLOAD_ARGS \
      model.moe_router_dtype=${OV2_ROUTER_DTYPE:-fp32} \
