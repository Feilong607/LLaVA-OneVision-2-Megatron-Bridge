...
#   0  PHASE-1 baseline  : pure bf16 + alltoall; recompute OFF on GB200 (192GB; == AIAK numerically, faster), ON on A-cards. DEFAULT mode.
...
#
# GB200 knobs:
#   FREE:    192GB HBM -> recompute OFF for ALL ACCEL modes on GB200 (DISABLE_RECOMPUTE=1 default, incl ACCEL=0); expandable_segments; NVLS.
...
if [[ "$ACCEL" == "1" ]]; then          # Phase-2a: MXFP8 + alltoall (fp8 HW only)
  ...
  if [[ "$HWNAME" == "gb200" ]]; then
    DISABLE_RECOMPUTE="${DISABLE_RECOMPUTE:-1}"; OV2_RECOMPUTE_FULL="${OV2_RECOMPUTE_FULL:-0}"   # GB200: recompute OFF (set DISABLE_RECOMPUTE=0 to restore AIAK full/uniform/1 if you ever OOM)
  else
...
OVERRIDES="$OVERRIDES dist.distributed_timeout_minutes=${OV2_DIST_TIMEOUT_MIN:-60}"
