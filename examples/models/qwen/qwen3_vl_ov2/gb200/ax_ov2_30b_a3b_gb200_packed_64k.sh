...
#
# GB200 knobs:
#   FREE:    192GB HBM -> recompute OFF for ACCEL=1/2 on GB200 (DISABLE_RECOMPUTE=1); ACCEL=0 keeps AIAK recompute; expandable_segments; NVLS.
...
case "$HW" in
  ...
  auto) _cc="$(nvidia-smi --query-gpu=compute_cap --format=csv,noheader 2>/dev/null | head -1 | tr -d ' .' || true)";;   # || true: on multi-GPU nodes head -1 closes the pipe while nvidia-smi is still writing -> SIGPIPE(141) -> pipefail+set -e SILENT death; empty _cc falls back below
  *) _cc="";;
esac
_cc="${_cc:-80}"
