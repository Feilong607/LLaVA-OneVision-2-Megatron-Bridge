...
MIDTRAIN_GBS="${OV2_MIDTRAIN_GBS:-64}"                   # global batch size; override with OV2_MIDTRAIN_GBS=
...
LOG_EVERY="${LOG_EVERY:-1}"; SAVE_EVERY="${SAVE_EVERY:-6000}"
...
# MoE token capacity control. GB200 has enough memory, so default to no token dropping / closest
...
if [[ "${HWNAME:-}" == "gb200" || -d /datasets/qwen-models-ea5jyi ]]; then
  ...
  OV2_HF_PROC_30B="${OV2_HF_PROC_30B:-/home/ftan0055/llava-ov2-30b-a3b-m9lvdn/auto_model}"                 # bundled processor (GB200)
  OV2_HF_PROC_30B_P16M33="${OV2_HF_PROC_30B_P16M33:-/home/ftan0055/llava-ov2-30b-a3b-m9lvdn/auto_model}"   # bundled processor (GB200, p16m33 recipe)
  ...
  INIT_CKPT="${INIT_CKPT:-/home/ftan0055/llava-ov2-30b-a3b-m9lvdn}"   # trained ckpt to resume (GB200; has iter_0001000 + auto_model)
  SAVE="${SAVE:-/home/ftan0055/ckpts_video_sft/ov2_30b_a3b_gb200_8gpu}"      # output dir (GB200, user-set)
...
ACCEL="${ACCEL:-2}"
# export OV2_HYBRIDEP_NUM_SMS=16
# export OV2_EP_OVERLAP=1
# export OV2_HYBRIDEP_PERMUTE_FUSION=1
export OV2_MOE_ROUTER_PAD_FP8=0
...
if [[ "$ACCEL" == "1" ]]; then          # Phase-2a: MXFP8 + alltoall (fp8 HW only)
  ...
  if [[ "$HWNAME" == "gb200" ]]; then
    DISABLE_RECOMPUTE="${DISABLE_RECOMPUTE:-0}"; OV2_RECOMPUTE_FULL="${OV2_RECOMPUTE_FULL:-1}"; OV2_RECOMPUTE_MOE="${OV2_RECOMPUTE_MOE:-0}"   # [OCR-UNCERTAIN: FULL/MOE 的 0/1 像素难辨] GB200: SELECTIVE recompute (core_attn + MoE layer) -- recomputes only the big MoE activations, far cheaper than full/uniform yet fits 192GB. OV2_RECOMPUTE_FULL=1 for AIAK full parity; DISABLE_RECOMPUTE=1 to turn off.
  else
...
#   change every restart!). GPUS_PER_NODE = $NPROC (GB200=4 from the HW profile above; A100/H100=8).
#   Priority:
...
# (K8s downward API) or OV2_K8S_NAMESPACE; defaults to 'runai-mv0004'.   # [OCR-UNCERTAIN: 此注释行只截到后半]
...
export PYTHONPATH="$REPO/_verify_stubs:/home/ftan0055/pylibs:$REPO/src:$REPO/3rdparty/Megatron-LM:$REPO/aiak_shim${PYTHONPATH:+:$PYTHONPATH}"
...
# NVLink. Re-add CollNet ONLY if your gb200_nccl_test passes WITH it: NCCL_ALGO=...,CollnetDirect,CollnetChain.
...
export HYBRID_EP_MAX_TOKENS_PER_RANK=10240
...
# defaults them off for the midtrain AdamW branch, so this is a functional no-op that GUARANTEES GB200 never
