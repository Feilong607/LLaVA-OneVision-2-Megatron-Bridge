#!/usr/bin/env bash
# =============================================================================
# OV2-30B-A3B (Qwen3-30B-A3B MoE + OV2 vision encoder + m33 adapter) · Stage-1 alignment
...
REPO="${REPO:-/ov2/feilong/gb200/Megatron-Bridge-refactor}"   # OV2 multi-backbone code + ov2_35b_a3b_* recipes
...
SAVE="${SAVE:-/ov2/feilong/gb200/ckpts_video_sft/ov2_30b_a3b_stage1}"
NPROC="${NPROC:-8}"
...
mkdir -p "$SAVE"; docker rm -f ov2_30b_s1 2>/dev/null || true
echo "[ov2-30b-stage1] nnodes=${NNODES:-1} save=$SAVE repo=$REPO"
docker run -d --name ov2_30b_s1 --network=host --privileged --gpus all -e CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 \
  ...
  -v /ov2:/ov2 -v /vlm:/vlm -w "$REPO" "$IMAGE" bash -lc "
    python -m torch.distributed.run $RDZV --nproc_per_node=$NPROC scripts/training/run_recipe.py \
      --recipe ov2_35b_a3b_stage1 --dataset vlm-energon --step_func ov2_step \
