...
def sync_hybrid_device_optimizer_fp32_master_copies(optimizer: MegatronOptimizer | None) -> bool:
    ...
    Ported from upstream Megatron-Bridge PR #4082. Workaround for an upstream Megatron-Core gap:
    when a checkpoint is loaded into the BF16 model parameters, ``reload_model_params()`` only
    refreshes the level-1 FP32 GPU shards inside ``HybridDeviceOptimizer``.  The level-2
