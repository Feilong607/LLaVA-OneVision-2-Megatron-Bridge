...
"""Vision-tower TransformerBlock for the OV2.1 onevision encoder.

Ported verbatim from
``aiak_training_llm.models.llava_onevision2.vision_transformer_block`` — the
source file imports nothing from ``aiak_training_llm`` so no transformations
were needed beyond moving it under the qwen_vl_ov2 package.
"""
...
class TransformerBlock(MegatronTransformerBlock):
    def __init__(self, *args, **kwargs):
        ...
        # Bridge-vendored upstream Megatron-LM no longer initialises self.cuda_graphs
        # on TransformerBlock; the OV2 forward below still references the attribute.
        # Default to empty (no CUDA graphs) - matches the legacy mcore behaviour.
